...
// ImagePro_PDoc.cpp: CImageProPDoc 클래스의 구현
//
...
IMPLEMENT_DYNCREATE(CImageProPDoc, CDocument)

BEGIN_MESSAGE_MAP(CImageProPDoc, CDocument)
	ON_COMMAND(ID_SUM_CONSTANT, &CImageProPDoc::OnSumConstant)
	ON_COMMAND(ID_SUB_CONSTANT, &CImageProPDoc::OnSubConstant)
	ON_COMMAND(ID_MUL_COSTANT, &CImageProPDoc::OnMulCostant)
	ON_COMMAND(ID_DIV_CONSTANT, &CImageProPDoc::OnDivConstant)
	ON_COMMAND(ID_AND_OPERATE, &CImageProPDoc::OnAndOperate)
	ON_COMMAND(ID_OR_OPERATE, &CImageProPDoc::OnOrOperate)
	ON_COMMAND(ID_XOR_OPERATE, &CImageProPDoc::OnXorOperate)
	ON_COMMAND(ID_NEGA_TRANSFORM, &CImageProPDoc::OnNegaTransform)
	ON_COMMAND(ID_GAMMA_CORRECTION, &CImageProPDoc::OnGammaCorrection)
	ON_COMMAND(ID_BINARIZATION, &CImageProPDoc::OnBinarization)
	ON_COMMAND(ID_STRESS_TRANSFORM, &CImageProPDoc::OnStressTransform)
	ON_COMMAND(ID_HISTO_STRETCH, &CImageProPDoc::OnHistoStretch)
	ON_COMMAND(ID_END_IN_SEARCH, &CImageProPDoc::OnEndInSearch)
	ON_COMMAND(ID_HISTOGRAM, &CImageProPDoc::OnHistogram)
	ON_COMMAND(ID_HISTO_EQUAL, &CImageProPDoc::OnHistoEqual)
	ON_COMMAND(ID_HISTO_SPEC, &CImageProPDoc::OnHistoSpec)
	ON_COMMAND(ID_EMBOSSING, &CImageProPDoc::OnEmbossing)
	ON_COMMAND(ID_BLURR, &CImageProPDoc::OnBlurr)
	ON_COMMAND(ID_GAUSSIAN_FILTER, &CImageProPDoc::OnGaussianFilter)
	ON_COMMAND(ID_SHARPENING, &CImageProPDoc::OnSharpening)
	ON_COMMAND(ID_HPF_SHARP, &CImageProPDoc::OnHpfSharp)
	ON_COMMAND(ID_LPF_SHARP, &CImageProPDoc::OnLpfSharp)
	ON_COMMAND(ID_DIFF_OPERATOR_HOR, &CImageProPDoc::OnDiffOperatorHor)
	ON_COMMAND(ID_HOMOGEN_OPERATOR, &CImageProPDoc::OnHomogenOperator)
	ON_COMMAND(ID_LAPLACIAN, &CImageProPDoc::OnLaplacian)
	ON_COMMAND(ID_NEAREST, &CImageProPDoc::OnNearest)
	ON_COMMAND(ID_BILINEAR, &CImageProPDoc::OnBilinear)
	ON_COMMAND(ID_MEDIAN_SUB, &CImageProPDoc::OnMedianSub)
	ON_COMMAND(ID_MEAN_SUB, &CImageProPDoc::OnMeanSub)
END_MESSAGE_MAP()


// CImageProPDoc 생성/소멸

CImageProPDoc::CImageProPDoc() noexcept
{
	// TODO: 여기에 일회성 생성 코드를 추가합니다.

}

CImageProPDoc::~CImageProPDoc()
{
}

BOOL CImageProPDoc::OnNewDocument()
{
...
// CImageProPDoc serialization

void CImageProPDoc::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
...
// 축소판 그림을 지원합니다.
void CImageProPDoc::OnDrawThumbnail(CDC& dc, LPRECT lprcBounds)
{
...
// 검색 처리기를 지원합니다.
void CImageProPDoc::InitializeSearchContent()
{
...
void CImageProPDoc::SetSearchContent(const CString& value)
{
	if (value.IsEmpty())
	{
...
#ifdef _DEBUG
void CImageProPDoc::AssertValid() const
{
	CDocument::AssertValid();
}

void CImageProPDoc::Dump(CDumpContext& dc) const
{
	CDocument::Dump(dc);
}
#endif //_DEBUG
...
BOOL CImageProPDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
...
BOOL CImageProPDoc::OnSaveDocument(LPCTSTR lpszPathName)
{
	CFile File; // 파일 객체 선언
...
void CImageProPDoc::OnDownSampling()
{
	int i, j;
	CDownSampleDlg dlg;
...
void CImageProPDoc::OnUpSampling()
{
	int i, j;
...
void CImageProPDoc::OnQuantization()
{
...
void CImageProPDoc::OnSumConstant()
{
...
void CImageProPDoc::OnSubConstant()
{
	CConstantDlg dlg;
...
void CImageProPDoc::OnMulCostant()
{
	CConstantDlg dlg;
...
void CImageProPDoc::OnDivConstant()
{
	CConstantDlg dlg;
...
void CImageProPDoc::OnAndOperate()
{
	CConstantDlg dlg;
	int i;
...
void CImageProPDoc::OnOrOperate()
{
	CConstantDlg dlg;
	int i;
...
void CImageProPDoc::OnXorOperate()
{
	CConstantDlg dlg;
	int i;
...
void CImageProPDoc::OnNegaTransform()
{
	int i;
...
void CImageProPDoc::OnGammaCorrection()
{
	CConstantDlg dlg;
...
void CImageProPDoc::OnBinarization()
{
	CConstantDlg dlg;
...
void CImageProPDoc::OnStressTransform()
{
...
void CImageProPDoc::OnHistoStretch()
{
	int i;
...
void CImageProPDoc::OnEndInSearch()
{
	int i;
	unsigned char LOW, HIGH, MAX, MIN;
...
void CImageProPDoc::OnHistogram()
{
	// 히스토그램의 값은 0~255
...
void CImageProPDoc::OnHistoEqual()
{
	int i, value;
...
void CImageProPDoc::OnHistoSpec()
{
...
void CImageProPDoc::OnEmbossing()
{
	int i, j;
...
double** CImageProPDoc::OnMaskProcess(unsigned char* Target, double Mask[3][3])
{
	// 회선 처리가 일어나는 함수
	int i, j, n, m;
...
double** CImageProPDoc::OnScale(double** Target, int height, int width)
{
	// 정규화를 위한 함수
	int i, j;
	double min, max;
...
double** CImageProPDoc::Image2DMem(int height, int width)
{
	// 2차원 메모리 할당을 위한 함수
	double** temp;
	int i, j;
...
void CImageProPDoc::OnGaussianFilter()
{
	int i, j;
...
void CImageProPDoc::OnSharpening()
{
	int i, j;
...
void CImageProPDoc::OnDiffOperatorHor()
{
	int i, j;
...
void CImageProPDoc::OnHomogenOperator()
{
	int i, j, n, m;
...
double CImageProPDoc::DoubleABS(double X)
{
	// 실수의 절대 값 연산 함수
	if (X >= 0)
		return X;
...
void CImageProPDoc::OnLaplacian()
{
	int i, j;
...
void CImageProPDoc::OnMedianSub()
{
...
void CImageProPDoc::OnBubleSort(double* A, int MAX)
{
	// 데이터의 정렬을 처리하는 함수
	int i, j;
	for (i = 0; i < MAX; i++) {
...
void CImageProPDoc::OnSwap(double* a, double* b)
{
	// 데이터 교환 함수
	double temp;
	temp = *a;
