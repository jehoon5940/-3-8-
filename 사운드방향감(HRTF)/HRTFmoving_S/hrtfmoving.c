...
void Convolution8(unsigned char* pcm_data, long data_size, unsigned char* directed_pcm_data)
{
...
void main()
{
	...
	step = (long)(D.chunkSize/72);
	for(i=0, AZIM=0;AZIM<720;i+=step, AZIM+=10){
		GetHRTF(ELEV, AZIM);
		Convolution8(&(pcm_data[i]), step, &(directed_pcm_data[2*i]));
	}

	sprintf(nameout, "moving.wav");
